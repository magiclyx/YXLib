...
static yx_core_eventloop g_eventloop;
...
yx_bool yx_core_setup(int argc, const char** argv)
{
    ...
    /*
     设置全局runloop
     */
    yx_core_eventloop_init(yx_get_allocator(), &g_eventloop);
    yx_core_eventloop_setGlobal(&g_eventloop);
...
yx_bool yx_core_shutdown(void)
{
    ...
    /*
     清除全局runloop
     */
    yx_core_eventloop_setGlobal(NULL);
    yx_core_eventloop_recycle(&g_eventloop);
